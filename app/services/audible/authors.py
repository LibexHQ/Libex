...
def _mint_screen_token(page_num: int) -> str:
    ...
    Verified live against the real endpoint (see
    LIBEX_ISSUE_77_REFERENCE.md and LIBEX_ISSUE_77_INVESTIGATION.md): a
...
# (audimeta-compat verified live that today's response has the
# -ReleaseDate name-search list as a byte-identical prefix, and existing
# consumers must keep seeing that same list, in that same order, unmoved
# at the front of the union -- see get_author_books). Ascending
...
async def get_author_books(
    ...
    List order is a consumer-visible contract: audimeta-compat verified
    live that today's response has the -ReleaseDate catalog list as a
    byte-identical prefix, and existing consumers must keep seeing that
    same list, in that same order, unmoved at the front of this union.
