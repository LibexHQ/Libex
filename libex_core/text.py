"""
Text and image-URL cleanup for Audible's raw product data.

Audible's own fields carry artifacts that are theirs, not the domain's --
embedded HTML markup, escaped control sequences, and image URLs pinned to a
particular render size. These functions strip that noise so normalized
output reflects the content rather than the delivery format it arrived in.
"""
